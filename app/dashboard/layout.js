    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-app-bg dark:bg-app-bg-dark">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm font-semibold text-app-text-muted">{navbar.loading}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex flex-col min-h-screen bg-app-bg dark:bg-app-bg-dark text-app-text dark:text-app-text-dark font-sans selection:bg-primary-muted selection:text-primary">
      ...
      <div className="fixed top-4 md:top-6 left-0 right-0 z-[60] flex justify-center px-2 sm:px-4 pointer-events-none">
        <nav className="pointer-events-auto flex items-center gap-1 sm:gap-2 p-1.5 bg-app-surface/70 dark:bg-app-surface-dark/70 backdrop-blur-2xl border border-slate-200/50 dark:border-white/10 rounded-full shadow-[0_8px_32px_rgba(0,0,0,0.12)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.3)] max-w-full overflow-hidden">
          ...
          <Link href="/" className="flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gradient-to-tr from-primary to-accent text-white font-bold text-lg sm:text-xl ml-0.5 mr-1 shadow-[inset_0_1px_rgba(255,255,255,0.4)] shrink-0 transition-transform active:scale-90">
            {navbar.logoLabel}
          </Link>

          <div className="w-px h-6 bg-app-text-muted/10 dark:bg-white/10 mx-0.5" />

          <Link href="/dashboard" className="group flex items-center px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm font-bold tracking-tight rounded-full text-app-text-muted dark:text-app-text-dark/70 hover:text-app-text dark:hover:text-app-text-dark hover:bg-app-bg dark:hover:bg-white/10 transition-all active:scale-95 whitespace-nowrap">
          ...
          <Link href="/dashboard/subscriptions" className="group flex items-center px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm font-bold tracking-tight rounded-full text-app-text-muted dark:text-app-text-dark/70 hover:text-app-text dark:hover:text-app-text-dark hover:bg-app-bg dark:hover:bg-white/10 transition-all active:scale-95 whitespace-nowrap">
            <svg className="w-5 h-5 md:mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
            </svg>
            <span className="hidden md:inline">{navbar.links.subscriptions}</span>
          </Link>
